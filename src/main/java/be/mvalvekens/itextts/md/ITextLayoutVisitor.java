...
public class ITextLayoutVisitor extends AbstractVisitor {
    private final IDocumentContext context;
    ...
    public ITextLayoutVisitor(
            IDocumentContext context, Consumer<? super com.itextpdf.layout.element.Paragraph> paraConsumer,
    ...
    public void visit(Link link) {
        ...
        if (dest.charAt(0) == '!') {
            // syntax hack to put in custom roles through markdown
            String customRole = dest.substring(1);
            this.currentPara.add(visitSpan(link, customRole, null, null));
            return;
